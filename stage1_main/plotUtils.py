...
lake_feat_names = ['sat_hypo', 'thermocline_depth', 'temperature_epi', 'temperature_hypo', 'volume_epi', 'volume_hypo', 'fnep',
    ...
    for i in range(0, feature_num):
        for j in range(i + 1, feature_num):
            if beta_data[i][j] == 0.:
                ...
                if beta_data[i][j] > 0.99999:
                    beta_data[i][j] = 0.9
                elif beta_data[i][j] > 0.999:
                    beta_data[i][j] = 0.85
                elif beta_data[i][j] > 0.99:
                    beta_data[i][j] = 0.72
                else:
                    beta_data[i][j] = beta_data[i][j] * 0.65
...
def heat_map_plot(alpha_data, beat_data, feat_names, index, dataset, experiment_time):
    ...
    sns.heatmap(beat_data, vmin=-1, vmax=3.9, xticklabels=feat_names, yticklabels=feat_names, square=True,
                cmap=beta_colormap,
                annot=np.array(beat_data, dtype='int'), annot_kws={'size': 16, 'weight': 'bold', 'color': 'black'},
                cbar=False, ax=ax1)
    ...
    img_path = os.path.join(dir_path, str(int(index)) + '.png')
...
def fis_plot(dataset, feat_name, experiment_time):
    ...
    dir_traversal(os.path.join('../param', dataset, experiment_time, "evolution", 'operation_type'),
                  operation_type_path_list)
    alpha_beta_path_list = []
    dir_traversal(os.path.join('../param', dataset, experiment_time,
                  'evolution', 'alpha_beta'), alpha_beta_path_list)
    print("alpha_beta_path_list:", len(alpha_beta_path_list))
    for index in tqdm(range(len(alpha_beta_path_list))):
        ...
        operation_type_data = load_operation_data(
            operation_type_file_path, feat_name)
        alpha_data, beta_data = load_alpha_beta_data(
            alpha_beta_file_path, feat_name)
        beta_data = cal_beta_data(
            operation_type_data, beta_data, len(feat_name))

        heat_map_plot(alpha_data, beta_data, feat_name,
                      index[-1], dataset, experiment_time)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Plot Foundation Model example')
    choices = list(itertools.product(['lake'], [
                   '1,1', '1+1', '1+n', 'n,1', 'n+1']))
    parser.add_argument('--dataset_strategy', type=str, default='lake_n,1', help='use model',
                        choices=[f'{x}_{y}' for x, y in choices])
    ...
    if dataset == 'criteo_1,1' or dataset == 'criteo_1+1' or dataset == 'criteo_1+n' or dataset == 'criteo_n,1' or dataset == 'criteo_n+1':
        ...
        feat_name = lake_feat_names
        print("feat_name shape:", len(lake_feat_names))
    else:
