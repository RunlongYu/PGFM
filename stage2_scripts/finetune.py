...
from config.temp_config import fmPgConfig, fmLSTMConfig, lstmConfig, fmTFConfig, transformerConfig, ealstmConfig
from utils.function_utils import build_input_features
from utils.utils import USE_FEATURES, UNSUP_TEMP_FEATURES, STATIC_FEATURES
from utils.utils import create_path, getHypsographyManyLakes
...
from utils.utils_stage2 import getPbTemperatureResults
...
def train(args):
    ...
    save_path = create_path(
        f'../stage2_results/{model_type}/{current_time}/{lake_id}/{seed}/')

    # Init log
    filename = str(model_type) + '_' + strategy + '_' + \
        '_result_' + str(time.strftime("%H-%M-%S", time.localtime())) + f'seed_{seed}' + '.log'
    logging.basicConfig(filename=os.path.join(
        f'../stage2_results/{model_type}/{current_time}/', filename), level=logging.INFO, filemode='w')
    ...
    print("trn_label.shape:", trn_label.shape)
    print("trn_sim_label.shape", trn_sim_label.shape)
    assert trn_label.shape == trn_sim_label.shape
    assert test_label.shape == test_sim_label.shape
    assert val_label.shape == val_label.shape
    ...
    if model_type == 'lstm':
        ...
        train_config = lstmConfig
        test_results = getPbTemperatureResults(x=test_data, sim_pred_y = test_sim_label, y=test_label, phys_data=test_phys_data,
    ...
    model.before_train(trainConfig=train_config)
    set_seed(seed=seed)
    model.fit_stage2(x=trn_data, y=trn_label, unsup_x=test_data, phy_data=test_phys_data, val_x=val_data, val_y=val_label, hypsography = hypsography,
                                batch_size=train_config['batch_size'],
                                epochs=train_config['train_epochs'],config=train_config,
                                shuffle=False, use_gpu=True)

    ###### SAVE Results #####
    test_results = model.predict_temp(x=test_data, y=test_label, phys_data=test_phys_data,
                  hypsography=hypsography, config=train_config, dataset='test', save_path=save_path)

    train_results = model.predict_temp(x=trn_data, y=trn_label, phys_data=trn_phys_data,
                  hypsography=hypsography, config=train_config, dataset='train', save_path=save_path)

    val_results = model.predict_temp(x=val_data, y=val_label, phys_data=val_phys_data,
    ...
    save_data(os.path.join(save_path, 'val_phys.npy'), val_label)
