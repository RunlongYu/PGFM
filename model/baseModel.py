...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, initial_epoch=0, validation_split=0.,
        ...
        train_tensor_data = TensorDataset(torch.from_numpy(x), torch.from_numpy(y))
    ...
    def predict(self, x, batch_size=256):
        ...
        with torch.no_grad():
            for index, x_test in enumerate(test_loader):
                ...
                y_pred_1, y_pred_2 = model(x) ### Changed
                # print("y_pred in function:", y_pred.shape)

                y_pred = torch.cat((y_pred_1.unsqueeze(-1), y_pred_2.unsqueeze(-1)), dim=-1).squeeze().cpu().data.numpy()
        ...
        if metrics:
            for metric in metrics:
                ...
                if metric == "mse":
                    metrics_[metric] = mean_squared_error
                if metric == "rmse":
                    metrics_[metric] = partial(mean_squared_error, squared=False)
