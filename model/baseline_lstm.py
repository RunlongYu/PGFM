...
class LSTM(nn.Module):
    ...
    def init_hidden(self, batch_size=0):
        ...
        h0 = xavier_normal_(torch.empty(1, batch_size, self.hidden_size)).to(self.device)
        c0 = xavier_normal_(torch.empty(1, batch_size, self.hidden_size)).to(self.device)
        return (h0, c0)

    def forward(self, x, hidden):
        self.lstm.flatten_parameters()
        x = x.float()
        hidden_layer, hidden = self.lstm(x, self.hidden)
        self.hidden = hidden
        self.hidden_layer = hidden_layer
        x = self.out(hidden_layer)
        x = F.relu(x)
        return x, hidden
    
    def get_liner_out(self, hidden_layer):
        x = self.out(hidden_layer)
        return x
    ...
    def fit_stage2(self, x=None, y=None, unsup_x = None, phy_data = None, hypsography = None, val_x=None, val_y=None, batch_size=None, epochs=1, initial_epoch=0,
            ...
            for epoch in range(epochs):
                ...
                for i, batches in  tqdm(enumerate(multi_loader)):
                        ...
                        y_pred, h_state = model(inputs, hidden = None)
                        ...
                        model.hidden = model.init_hidden(batch_size=unsup_inputs.size()[0])
                        unsup_outputs, h_state = model(unsup_inputs, hidden=None)
    ...
    def predict_temp(self, x, y, phys_data, hypsography, config, dataset, save_path):
        ...
        with torch.no_grad():
            for index, (x_test, y_test, phys_data) in enumerate(test_loader):
                ...
                model.hidden = model.init_hidden(batch_size=input.size()[0])
                y_pred, h_state = model(input, None)
