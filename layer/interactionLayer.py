...
class MixedOp(nn.Module):
    def __init__(self, input_dim, output_dim, device='cpu'):
        ...
        for interaction_type in Interaction_Types:
            op = Interation_Operations_Dict[interaction_type](
                input_dim=input_dim)
            self._ops.append(op)
    ...
    def forward(self, x1, x2, weights):
        sum = torch.zeros(
            (x1.shape[0], x1.shape[1], self.output_dim)).to(self.device)
...
class InteractionLayer(nn.Module):
    def __init__(self, input_dim, feature_columns, selected_interaction_type,
        ...
        if beta == None:
            self.beta = self.create_structure_param(
                len(selected_interaction_type), init_mean=0.5, init_radius=0.001)
        else:
            self.beta = beta
        self.batch_norm = torch.nn.BatchNorm1d(
            len(selected_interaction_type), affine=False, momentum=0.01, eps=1e-3)
        self.mixed_operation = MixedOp(
            input_dim=input_dim, output_dim=interaction_fc_output_dim, device=device)
        ...
        self.expand_linear = nn.Linear(1, self.input_dim)

    def forward(self, X, mutation=False):
        batch_size, seq_length, feature_num = X.shape
        X_reshaped = X.view(batch_size * seq_length * feature_num, 1)
        # Apply the linear layer
        expanded = self.expand_linear(X_reshaped)

        embed_matrix = expanded.view(
            batch_size*seq_length, feature_num, self.input_dim)
        ...
        interaction_matrix = self.mixed_operation(
            embed_i, embed_j, self.mask_weight)

        normed_interaction_matrix = self.batch_norm(interaction_matrix)
        if self.use_beta:
            weighted_interaction_matrix = torch.mul(
                normed_interaction_matrix, self.activate(self.beta.unsqueeze(-1)))
        else:
    ...
    def generate_mask_weight(self):
        mask_weight = torch.zeros(
            (self.interaction_pair_number, len(Interation_Operations_Dict)))
        ...
        for index in range(self.interaction_pair_number):
            if abs(self.beta[index]) < self.mutation_threshold:
                if random.random() < self.mutation_probability:
                    ...
                    mutation_interaction_type = (
                        self.selected_interaction_type[index] + random.randint(1, 3)) % 4
