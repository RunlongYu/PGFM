...
class LinearLayer(nn.Module):
    def __init__(self, feature_columns, feature_index, init_std=0.0001, reduce_sum=True):
        ...
        self.embedding_dict = create_embedding_matrix(
            feature_columns, init_std, linear=True, sparse=False)
...
class NormalizedWeightedLinearLayer(nn.Module):
    def __init__(self, feature_columns, alpha=None, use_alpha=True,
        ...
        if alpha == None:
            self.alpha = self.create_structure_param(
                len(self.feature_columns), init_mean=0.5, init_radius=0.001)
        else:
            self.alpha = alpha
    ...
    def forward(self, X, keepdim=False):
        if self.use_alpha:
            if keepdim:
                linear_logit = torch.mul(X, (self.activate(self.alpha)))
            else:
                linear_logit = torch.sum(
                    torch.mul(X, (self.activate(self.alpha))), dim=-1,
                    keepdim=True)
        else:
            linear_logit = torch.sum(X, dim=-1, keepdim=True)
        return linear_logit
